...
export async function load({ url }) {
  ...
  // NB: join on d.season_id, not d.draft_id. Those ids happen to be equal for
  // 2015-2023, but 2024 is season 11 / draft 13 -- the old join dropped 2024 and
  // wrongly listed 2026 (season 13, which coincidentally equals draft 13).
  const availableSeasons = (await query(`
    SELECT DISTINCT s.season_year, s.season_id
    FROM seasons s
    JOIN drafts d ON d.season_id = s.season_id
