			...
			{:else}
				...
				<div class="status-dashboard">
					...
					{:else if dataStatus}
						...
						<!-- Missing Weeks Alert -->
						{@const missingWeeks = Array(14).fill(0).map((_, i) => i + 1).filter(w => 
							(dataStatus.matchups?.[w] || 0) === 0 && 
							(dataStatus.weeklyRosters?.[w] || 0) === 0 && 
							(dataStatus.playerStats?.[w] || 0) === 0
						)}
						{#if missingWeeks.length > 0}
							<div class="status-section">
								<div class="alert alert-warning" style="margin: 0;">
									<h4 style="margin: 0 0 0.5rem 0;">⚠️ Missing Regular Season Weeks</h4>
									<p style="margin: 0 0 0.5rem 0;">
										The following weeks have NO data imported (no matchups, rosters, or stats):
									</p>
									<div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
										{#each missingWeeks as week}
											<span class="week-badge missing">Week {week}</span>
										{/each}
									</div>
									<p style="margin: 0.75rem 0 0 0; font-size: 0.9em;">
										💡 Use the "Import Matchups" and "Archive Rosters & Stats" buttons below to import this data.
									</p>
								</div>
							</div>
						{/if}
