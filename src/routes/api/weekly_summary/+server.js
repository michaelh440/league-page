        ...
        const result = await query(`
            WITH weekly_matchup_details AS (
                SELECT 
                    ...
                    -- Team 1 info
                    m.team1_id,
                    COALESCE(mtn1.team_name, mgr1.username) as team1_name,
                    ...
                    -- Team 2 info
                    m.team2_id,
                    COALESCE(mtn2.team_name, mgr2.username) as team2_name,
                    ...
                    -- Winner calculation
                    CASE 
                        WHEN m.team1_score > m.team2_score THEN COALESCE(mtn1.team_name, mgr1.username)
                        WHEN m.team2_score > m.team1_score THEN COALESCE(mtn2.team_name, mgr2.username)
                        ELSE 'TIE'
                    END as winner,
                ...
                -- Join DIRECTLY to managers (team1_id = manager_id)
                JOIN managers mgr1 ON m.team1_id = mgr1.manager_id
                LEFT JOIN manager_team_names mtn1 ON mtn1.manager_id = mgr1.manager_id 
                    AND mtn1.season_year = s.season_year
                
                -- Join DIRECTLY to managers (team2_id = manager_id)
                JOIN managers mgr2 ON m.team2_id = mgr2.manager_id
